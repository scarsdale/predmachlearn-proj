...
```{r}
preproc <- preProcess(data$training, method=c("center", "scale"))
data$training <- predict(preproc, data$training)
data$testing <- predict(preproc, data$testing)
```
...
```{r}
candidate.algorithms <- c("parRF", "svmLinear", "nnet")
```
